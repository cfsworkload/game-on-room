...
1. As a Player: navigate through a network/maze of rooms, where each room is a unique implementation of a common API. Each room supports chat, and interaction with items, some of which might be in the room, some of which might be separately defined services.
...
This walkthrough will guide you through adding a room to a running Game On! microservices application.  You will be shown how to setup a container-based room in the Go language as well as use Node.js to create a Cloud Foundry application in Bluemix.
...
When deployed using an instant runtime, Gameon-room-nodejs requires the following accounts:
...
4.  You should now see your **Game On! ID** and **Shared Secret** near the bottom of the page.  If necessary, refresh the page, or even log out and log back in, to generate your **Shared Secret**.  Please make note of your **Game On! ID** and **Shared Secret** for later in the walkthrough.
...
### Configure a Go room
As part of the `docker run` command, your container will expect its parameters in the form of environment variables. An example env file, example-container.env, has been provided and you must edit it to customize your settings. The file itself and container-startup.sh contains commentary regarding each of the environment variables, but you might need to edit the following important settings:
...
- **GAMEON_ID** This is the **Game On! ID** provided by the microservices app after you log in using your browser. Click the **Player** icon in the upper right corner of the web page after logging in.
- **GAMEON_SECRET** This is the **Shared Secret** provided by the microservices application after you login using your browser. Click the **Player** icon in the upper right corner of the web page after logging in.
- **ROOM_NAME** Set this to the name of your room.

## Make sure a public IP is available in your Bluemix space
This solution, when using Containers, requires a free public IP address. In order to determine if a public IP is available, you need to find the number of used and the max quota of IP addresses allowed for your space.

To find this information:

1. Log in to Bluemix at [https://console.ng.bluemix.net](https://console.ng.bluemix.net).
2. Select **DASHBOARD**.
3. Select the space where you would like your Game On! room to run.
...
1. Log in to your Bluemix account and space.
...
To build a Game On! room in a container, first create the container locally in Docker using the provided Docker file, and then push the container onto your Bluemix container registry.
...
1. The container uses a startup script which is driven by environment variables. Edit example-container.env and change the environment variable to suit your needs as you will be using this to pass environment variable settings to the container.
...
3. Use the resulting container ID to bind to the IP you got earlier with the command:
